...
import { z } from 'zod';
import { parseIntent } from '@/contexts/ViewModeContext';
...
export async function detectIntent(input: string): Promise<IntentResponse> {
  ...
  if (!apiKey) {
    ...
    return createFallbackResponse(parseIntent(input));
  }
 ...
 */
function createFallbackResponse(mode: 'architect' | 'author' | 'lab'): IntentResponse {
  // Map modes to audio parameters
  const audioParamsMap: Record<string, { reverb: number; filter: number }> = {
